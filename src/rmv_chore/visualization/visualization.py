...
from library import VisualizationParams, FrameRMV, MarkerRmv, SharedData, CameraManager, CubeTransformer
...
class CameraProjector:
    @staticmethod
    def get_visible_corners(transformed_corners: np.ndarray, camera_manager: CameraManager) -> List[Tuple[int, int]]:
...
class SphereDrawer:
    @staticmethod
    def draw_sphere(image: np.ndarray, marker, camera_manager: CameraManager) -> None:
...
class CylinderDrawer:
    @staticmethod
    def draw_cylinder(image: np.ndarray, marker:MarkerRmv, camera_manager: CameraManager) -> None:
